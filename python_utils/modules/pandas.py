...
def create_func_values2idcs(
    values_unique: npt.NDArray[np.int64],
    handle_missing_values: Literal[False] = ...,
) -> Callable[[npt.NDArray[np.int64]], npt.NDArray[np.int64]]:
    pass
...
def create_func_values2idcs(
    ...
    [npt.NDArray[np.int64]], npt.NDArray[Union[np.int64, np.float64]]
]:
    pass


def create_func_values2idcs(
    values_unique: npt.NDArray[np.int64], handle_missing_values: bool = False
) -> Callable[
    [npt.NDArray[np.int64]], npt.NDArray[Union[np.int64, np.float64]]
]:
        ...
        A function that maps each value to its index in the given array.
            ...
            Returns:
                The indices of the values in the original values_unique array.
                    ...
                    Shape: [N]
    """
    if not handle_missing_values and np.issubdtype(
        values_unique.dtype, np.integer
    ):
        ...
        # not larger than 100M (which will cost 800MB).
        min_value = np.min(values_unique)
        max_value = np.max(values_unique)
        if max_value - min_value + 1 <= 100_000_000:
            value2idx = np.empty(max_value - min_value + 1, dtype=np.int64)
            value2idx[values_unique - min_value] = np.arange(
                len(values_unique)
            )
            return lambda x: value2idx[x - min_value]

    # If the values are not integers or missing values should be handled, we
    # will use a dictionary for remapping. Note that pd.Series.map will return
    # np.nan automatically if a value is not present in the dictionary.
...
def remap_series_to_idcs(
    series: pd.Series, values_unique: npt.NDArray = ...
) -> tuple[
    npt.NDArray,
    Callable[[npt.NDArray], npt.NDArray[np.int64]],
...
def remap_series_to_idcs(
    ...
    npt.NDArray,
    Callable[[npt.NDArray], npt.NDArray[Union[np.int64, np.float64]]],
...
def remap_series_to_idcs(
    series: pd.Series, values_unique: Optional[npt.NDArray] = None
) -> tuple[
    npt.NDArray,
    Callable[[npt.NDArray], npt.NDArray[Union[np.int64, np.float64]]],
    npt.NDArray[Union[np.int64, np.float64]],
